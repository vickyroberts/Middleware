...
exports.verifyUserMobile = function(req, res) {  
  // Set the client properties that came from the POST data
  if(req.body && req.body.mobile && req.body.mobilecode && req.decoded.uid)
  { 
    ...
    {    
      ...
      {        
        //Check if the mobile code passed by user is correct. Change the user status.
        var queryStr = "SELECT * FROM milkdelivery_master.sp_update_verifymobile($1,$2,$3,$4);";
        var paramsArr = new Array(req.decoded.uid, req.body.mobilecode, req.body.mobile, req.decoded.schema);
        conn.pgExecuteQuery(queryStr, paramsArr, clientConn, function(err, result){
            res.status(200);
            if(err)
            {
